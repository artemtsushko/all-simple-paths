...
data Graph a = Graph { graphNodes :: [Node a]
                     , graphEdges :: [Edge a]
                     } deriving (Eq,Show,Read)
newtype Node a = Node a deriving (Eq,Show,Read)
newtype Edge a = Edge (Node a, Node a) deriving (Eq,Show,Read)
...
neighbourhood graph node =
    mapMaybe (getAdjacentNode node) $ graphEdges graph
